...
def constructBayesNet(gameState: hunters.GameState):
    ...
    # smallest distance : 0 (eg. actual distance - 7 = 0), 
    # largest distance : MAX_NOICE + X_RANGE - 1 + Y_RANGE - 1 
    # eg. pacman at (0, 0), ghost at (X_RANGE - 1, Y_RANGE - 1), noise = MAX_NOISE
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if noisyDistance is None or ghostPosition == jailPosition:
            ...
            # if satisfy either condition, must satisty both condition
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghostPosition, p in self.beliefs.items():
            # P(X_t+1| E_1:t+1) = P(E_t+1| X_t+1) * sum ^ x_t _ (P(X_t| E_1:t) * P(X_t+1| X_t))
            # P(X_t+1| X_t) = 1 since ghost doesn't move in given test
            # and there exists only previous for state for every current state
            # P(E_t+1| X_t+1) = self.getObservationProb(observation, pacmanPosition, ghostPosition, jailPosition)
            # P(X_t| E_1:t) = p
            self.beliefs[ghostPosition]  = p * self.getObservationProb(observation, pacmanPosition, ghostPosition, jailPosition)
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPos in self.allPositions:
            if self.beliefs[oldPos]:
                newPosDist = self.getPositionDistribution(gameState, oldPos)
                for newPos, transitionProb in newPosDist.items():
                    updateBeliefs[newPos] += self.beliefs[oldPos] * transitionProb
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        uneven = self.numParticles - even * len(self.legalPositions)
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.allPositions:
            if belief[oldPos] != 0:
                newPosDist = self.getPositionDistribution(gameState, oldPos)
                for newPos, transitionProb in newPosDist.items():
                    updateBelief[newPos] += belief[oldPos] * transitionProb
        if belief.total() == 0:
            self.initializeUniformly(gameState)
            return
        self.particles = [updateBelief.sample() for _ in range(self.numParticles)]
        return
