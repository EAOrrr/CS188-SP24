    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        cloestPos, closetDistance = None, float('inf')
        for ghostPositionDistribution in livingGhostPositionDistributions:
            ...
            if dist < closetDistance:
                cloestPos, closetDistance = ghostPos, dist
        action, distAfterAction = None, float('inf')
        for a in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, a)
            dist = self.distancer.getDistance(successorPosition, cloestPos)
            if dist < distAfterAction:
                action, distAfterAction = a, dist
        return action
