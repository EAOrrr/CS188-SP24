...
def findModelUnderstandingCheck() -> Dict[Expr, bool]:
    ...
    assignment = pycoSAT(cnf_sentence)
    a.op = a.op.lower()
    return assignment
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    # A |= B (A entails B is true) iff B is true for every case of B
    # which means that there does not exist any case where
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # only consider the last time step for the current position
    # ignoring illegal moves: such as moving into walls or stopping
    pacman_current_pos = PropSymbolExpr(pacman_str, x, y, time=now)
    return pacman_current_pos % disjoin(possible_causes)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    #  Results of calling successorAxioms(...), describing how Pacman can end in various
    #         locations on this time step. Consider edge cases. Don't call if None.
    # edge case: time = 0, there is no possible transition from time -1 to time 0
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    # Where needed, use allLegalSuccessorAxioms for transitions 
    # since this is for regular Pacman transition rules.
    ...
    # query models
    # model 1: Pacman is at (x1,y1) at time t=1 
...
def positionLogicPlan(problem) -> List:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    #  Initial knowledge: Pacman’s initial location at timestep 0
    KB.append(PropSymbolExpr(pacman_str, x0, y0, time=0))
    # KB.append(pacmanSuccessorAxiomSingle(x0, y0, 0, walls_grid))
    for t in range(50):
        print('timestep: ', t)
...
def foodLogicPlan(problem) -> List:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    def foodSuccessorAxiomSingle(x, y, t):
        if t == 0:
            return None
        # no food cause: Pacman is at (x, y) at time t-1 and there is no food at (x, y) at time t-1
        possible_no_food_causes = PropSymbolExpr(pacman_str, x, y, time=t-1) | (~(PropSymbolExpr(food_str, x, y, time=t-1)))
        return (~PropSymbolExpr(food_str, x, y, time=t)) % possible_no_food_causes
    ...
    for t in range(50):
        ...
        # Add to KB: Transition Model sentences: call foodSuccessorAxiomSingle(...) for all possible food positions in food.
        KB.extend(foodSuccessorAxiomSingle(x, y, t+1) for x, y in food)
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        KB.append(PropSymbolExpr(actions[t], time=t))
        ...
        # if KB entails Pacman(x, y, t) then Pacman must be at (x, y)
        possible_locations = [(x, y) for x, y in non_outer_wall_coords if entails(conjoin(KB), PropSymbolExpr(pacman_str, x, y, time=t))]
        print(possible_locations)
        if not possible_locations:
            possible_locations = [(x, y) for x, y in non_outer_wall_coords if not entails(conjoin(KB), ~PropSymbolExpr(pacman_str, x, y, time=t))]
             
        print(possible_locations)
        agent.moveToNextState(actions[t])
        "*** END YOUR CODE HERE ***"
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        KB_ = conjoin(KB)
        possible_locations = [(x, y) for x, y in non_outer_wall_coords if entails(KB_, PropSymbolExpr(pacman_str, x, y, time=t))]
        if not possible_locations:
            possible_locations = [(x, y) for x, y in non_outer_wall_coords if not entails(KB_, ~PropSymbolExpr(pacman_str, x, y, time=t))]
        
        for x, y in non_outer_wall_coords:
            if entails(KB_, PropSymbolExpr(wall_str, x, y)):
                known_map[x][y] = 1
            if entails(KB_, ~PropSymbolExpr(wall_str, x, y)):
                known_map[x][y] = 0
        agent.moveToNextState(actions[t]) 
